...
class account_financial_statement(report_sxw.rml_parse, common_report_header):
    ...
    def __init__(self, cr, uid, name, context=None):
        ...
        self.localcontext.update({
            ...
            'get_liability': self._get_liability,
    ...
    def _get_total_equity(self, data):
        ...
        ctx = self.context.copy()
        account_obj = self.pool.get('account.account')
        equity_ids = account_obj.search(self.cr, self.uid, [('type', '=', 'view'), ('user_type.name', '=', 'Equity')])
        if equity_ids:
            result = account_obj.read(self.cr, self.uid, equity_ids, ['type','code','name','debit','credit','balance','parent_id','level','child_id'], ctx)
        return result and result[0]['balance']
    ...
    def lines(self, form, ids=None, done=None):
        ...
        accs = obj_account.search(self.cr, self.uid, [('type', '=', 'view'), ('user_type.name', '=', 'Asset View')])
        if accs:
            ids = [accs[0]]
        parents = ids
    ...
    def _get_lines(self, form, ids=None, done=None):
        obj_account = self.pool.get('account.account')
        if not ids:
            ids = self.ids
        if not ids:
        ...
        if form['filter'] == 'filter_period':
            ...
            ctx['date_to'] =  form['date_to']
        ctx['state'] = form['target_move']
        accs = obj_account.search(self.cr, self.uid, [('type', '=', 'view'), ('user_type.name', '=', 'Equity')])
        if accs:
            ids = [accs[0]]
        parents = ids
        child_ids = obj_account._get_children_and_consol(self.cr, self.uid, ids, ctx)
    ...
    def _get_liability(self, form, ids=None, done=None):
        obj_account = self.pool.get('account.account')
        if not ids:
            ids = self.ids
        if not ids:
        ...
        if form['filter'] == 'filter_period':
            ...
            ctx['date_to'] =  form['date_to']
        ctx['state'] = form['target_move']
        accs = obj_account.search(self.cr, self.uid, [('type', '=', 'view'), ('user_type.name', '=', 'Liability View')])
        if accs:
            ids = [accs[0]]
        parents = ids
        child_ids = obj_account._get_children_and_consol(self.cr, self.uid, ids, ctx)
...
report_sxw.report_sxw('report.account.financial.statement', 'account.account', 'addons_aug/ifrs_account/report/financial_position.rml', parser=account_financial_statement, header="internal")
