...
class journal_print(report_sxw.rml_parse, common_report_header):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if (data['model'] == 'ir.ui.menu'):
            ...
            self.query_get_clause += obj_move._query_get(self.cr, self.uid, obj='l', context=ctx)
    ...
    def _sum_debit(self, period_id=False, journal_id=False):
        ...
        if account_id:
            self.cr.execute('SELECT SUM(debit) FROM account_move_line l, account_move am '
                            'WHERE l.move_id=am.id AND am.state IN %s AND l.period_id IN %s AND l.journal_id IN %s AND l.account_id IN %s' + ' ',
                            (tuple(move_state), tuple(period_id), tuple(journal_id), tuple(account_id)))
        else:
            self.cr.execute('SELECT SUM(debit) FROM account_move_line l, account_move am '
    ...
    def lines(self, period_id, journal_id=False):
        ...
        if self.target_move == 'posted':
            move_state = ['posted']
        if account_id:
            self.cr.execute('SELECT l.id FROM account_move_line l, account_move am WHERE l.move_id=am.id AND am.state IN %s AND l.period_id=%s AND l.journal_id IN %s AND l.account_id IN %s' + ' ORDER BY '+ self.sort_selection + ', l.move_id',(tuple(move_state), period_id, tuple(journal_id), tuple(account_id)))
        else:
...
report_sxw.report_sxw('report.account.journal.period.print1', 'account.journal.period', 'addons/ifrs_account_extension/report/account_journal.rml', parser=journal_print, header='external')
report_sxw.report_sxw('report.account.journal.period.print.sale.purchase1', 'account.journal.period', 'addons/ifrs_account_extension/report/account_journal_sale_purchase.rml', parser=journal_print, header='external')
