...
#                             LEFT JOIN res_currency c on (l.currency_id=c.id) WHERE am.state IN %s AND l.period_id=%s AND l.journal_id=%s AND l.account_id IN %s'+' GROUP BY a.id, a.code, a.name,l.amount_currency,c.symbol, a.currency_id,l.currency_id', (tuple(move_state), period_id, journal_id), tuple(account_id))
#         else:
        self.cr.execute('SELECT a.currency_id, a.code, a.name, c.symbol AS currency_code, l.currency_id, l.amount_currency, SUM(debit) AS debit, SUM(credit) AS credit  \
                        from account_move_line l  \
                        LEFT JOIN account_move am ON (l.move_id=am.id) \
...
report_sxw.report_sxw('report.account.central.journal1', 'account.journal.period', 'addons/ifrs_account_extension/report/account_central_journal.rml', parser=journal_print, header='external')
